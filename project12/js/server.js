...
${isNewUser || hasNoValidData ? 
`【新用戶歡迎模式】
這是一位剛開始使用系統的新用戶，或是還沒有足夠遊戲記錄的用戶。請提供溫暖的歡迎訊息和鼓勵，而不是比較進步。` : 
`【進步分析模式】
請根據玩家的表現，提供一份充滿正能量的分析報告。`}
...
- 禁止：不要提到「進步」、「比上週」、「表現優異」等需要歷史數據比較的詞語。` :
`- 一般用戶第一句：可以說「您持續在進步中！」或「您的表現很棒！」
